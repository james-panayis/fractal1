...
public:
  ...
  ~ui_event_impl()
  {
    ...
    emscripten_set_wheel_callback("canvas", this, 1, nullptr);
...
private:
  ...
  static EM_BOOL f_window_size(int event_type, const EmscriptenUiEvent *e, void *user_data)
  {
    ...
    auto res = emscripten_get_element_css_size(ui->canvas_.c_str(), &w, &h);

    if (res != EMSCRIPTEN_RESULT_SUCCESS) return EM_FALSE;
  ...
  void init()
  {
    ...
    emscripten_set_wheel_callback("canvas", this, 1, f_scroll_wheel);
